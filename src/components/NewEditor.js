...
function NewEditor() {
 ...
 useEffect(() => {
    const editor = grapesjs.init({
     ...
     height: '100vh',
     width: '100%',
     plugins: [websitePlugin,basicBlockPlugin,formPlugin,navbarPlugin,textEditor,imageEditor,toolTip,customCode],
     storageManager: {
       id: 'gjs-',
       type: 'local',
       autosave: true,
      ...
      if (user) {
        ...
        try {
          const response = await fetch(
            `http://127.0.0.1:8000/webpages/savepage/${localStorage.getItem(
              "email"
            )}/`,
            {
              method: "POST",
          ...
          if (responseData.message === "Page saved successfully.") {
